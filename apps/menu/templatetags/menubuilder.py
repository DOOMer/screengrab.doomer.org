...
def get_items(menu, current_path, user):
    ...
    for i in MenuItem.objects.filter(menu__slug=menu).order_by('order'):
        ...
        if i.link_url == '/':
            i.link_url = ''
        elif i.link_url.startswith('/'):
            i.link_url = i.link_url[1:]
            
                
        if not i.login_required or ( i.login_required and user.is_authenticated() ):
            menuitems.append({'url': i.link_url, 'title': i.title, 'current': current, 'target_blank': i.target_blank, 'external': i.external_url})
    return menuitems
